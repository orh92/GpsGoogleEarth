 ...
 */
	...
	public Point3D add(Point3D gps, Point3D local_vector_in_meter) {
		Point3D newp = meter2polar(local_vector_in_meter);
		newp.add(gps);
		return newp;
	}
/**
 ...
 * @return
 */
	public Point3D meter2polar(Point3D v3d) {
		double x = Math.toDegrees(Math.asin(v3d.x()/radius));
		double y = Math.toDegrees(Math.asin(v3d.y()/(radius*lon)));
